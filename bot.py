...
def get_robota_ua():
    ...
    # Посилені заголовки для обходу захисту Robota.ua
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36",
    ...
    try:
        ...
        if response.status_code != 200:
            return f"🚀 **Robota.ua:**\n⚠️ Помилка доступу (Status {response.status_code}). Оновіть Cookie.\n\n"
            
        soup = BeautifulSoup(response.text, 'html.parser')
        # Robota.ua часто ховає дані в JSON всередині тегу <script>. 
        # Спробуємо спочатку звичайний пошук по картках.
        cards = soup.select('alliance-candidate-card') or soup.select('div[class*="cv-card"]')
        
        report = "🚀 **Robota.ua (Вінниця):**\n"
        if not cards:
            return report + "⚠️ Сторінка завантажилась, але картки не знайдено (спробуйте оновити Cookie).\n\n"
